WITH
	...
	MEMBER [Measures].[HasSales] AS 
		([Date Commissioned].[Calendar Time].CurrentMember.Parent.Parent.Parent, [Measures].[Commissioned Sales TC])
...
} ON COLUMNS,
{
	filter(
		(
			[Ship To Customer Location].[Location].[Location].MEMBERS//[Sales Geography].[Sales Geography].[District].&[E5030]
		)
		,[Measures].[HasSales] <> 0
	)
} ON ROWS
